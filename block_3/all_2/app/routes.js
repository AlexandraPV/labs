...
module.exports = function(app, passport) {
   ...
   app.get('/', function(req, res) {
    ...
    app.delete('/deleteprodA*',isLoggedIn,  (req, res) => {

  	    var value = req.url;
  	    value = value.slice(21);
  	    //var bar = value.slice(0, 1).toUpperCase() +  value.slice(1);
  	       //  var name = req.params.brand_name;
  	     console.log(value);

  	    	db.collection('brands').remove({"name": value})
      ...
      .catch(err => res.status(404).json({ error: "ERROR" }));
        ...
        app.post('/apiproductsupdate/*', function(req, res, next) {
          ...
          if(mas[1] == "staf"){
            console.log(mas);
            var a = parseInt(mas[2]);
          db.collection('brands').update({name : mas[0]}, {$set: {staf: a}})
                .then(brand =>res.json(brand))
                  .catch(err => res.status(404).json({ error: "ERROR" }));
          }
          if(mas[1] == "cost"){
            var a = parseFloat(mas[2]);
            db.collection('brands').update({name : mas[0]}, {$set: {cost : a}})
                  .then(brand =>res.json(brand))
                    .catch(err => res.status(404).json({ error: "ERROR" }));
            }
